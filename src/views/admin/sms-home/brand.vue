<template>
    <div>
        ...
        <div class="down">
            <div>
                <!--列表组件-->
                <ListView :data="data" :column="column" title="首页热销品牌列表">
                    <slot>
        ...
        <!--新建/编辑的弹窗-->
        <el-dialog v-model="visible" v-if="visible" width="30%" title="首页热销品牌信息">
            <Brand :id="choiceId" @doCancel="doCancel"></Brand>
        </el-dialog>
    </div>
</template>
<script>
import ListView from '@/components/list-view/list-view.vue';
import { page, del, getHomeBrandsByRecommendStatus } from "@/http/sms/home/brandApi";
import Msg from "@/utils/msg/msg"
import Brand from '../../../components/sms/brand.vue';
export default {
    mounted() {
        //初始化设定列表每列显示的属性和名称
    ...
    methods: {
        //查询
        doQuery() {
            getHomeBrandsByRecommendStatus(this.queryEntity).then(resp => {
                console.log('查询收到的:' + JSON.stringify(resp))
                this.data = resp.data.smsHomeBrandList
            })
        },
        //编辑
        doEdit(id) {
        ...
        buildColumn() {
            return [
                ...
                { prop: "brandName", label: "品牌名称", width: 150 },
                { prop: "brandId", label: "点击次数", width: 80 },
            ];
        }
    },
    components: { ListView, Brand }
}
</script>
<style  scoped>
.img {
