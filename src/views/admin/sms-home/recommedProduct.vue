<template>
    <div>
        ...
        <div class="down">
            <div>
                <!--列表组件-->
                <ListView :data="data" :column="column" title="首页促销商品列表">
                    <slot>
        ...
        <!--新建/编辑的弹窗-->
        <el-dialog v-model="visible" v-if="visible" width="30%" title="首页促销商品信息">
            <RecommendProduct :id="choiceId" @doCancel="doCancel"></RecommendProduct>
        </el-dialog>
    </div>
</template>
<script>
import ListView from '@/components/list-view/list-view.vue';
import { page, del, getHomeRecommentProductsByRecommendStatus } from "@/http/sms/home/recommendProductApi";
import Msg from "@/utils/msg/msg"
import RecommendProduct from '../../../components/sms/recommendProduct.vue';
export default {
    mounted() {
        //初始化设定列表每列显示的属性和名称
    ...
    methods: {
        //查询
        doQuery() {
            getHomeRecommentProductsByRecommendStatus(this.queryEntity).then(resp => {
                console.log('查询收到的:' + JSON.stringify(resp))
                this.data = resp.data.smsHomeRecommendProducts
            })
        },
        //编辑
        doEdit(id) {
        ...
        buildColumn() {
            return [
                ...
                { prop: "sort", label: "点击次数", width: 100 },
                { prop: "productName", label: "商品名称", width: 600 },

            ];
        }
    },
    components: { ListView, RecommendProduct }
}
</script>
<style  scoped>
.img {
